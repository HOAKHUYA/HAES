...
*
*
*		Id: aes.module.php [BuildDB.665000254] 12/11/2020 8:03 CH $
*/
...
class aes extends HKAES {
#_________________________________________________________________________________________

  public static function hencrypt($plaintext, $password=iAES, $nBits=256) {
    $blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
    if (!($nBits==128 || $nBits==192 || $nBits==256)) return '';  // standard allows 128/192/256 bit keys
    $nBytes = $nBits/8;  // no bytes in key
    $pwBytes = array();
    for ($i=0; $i<$nBytes; $i++) $pwBytes[$i] = ord(substr($password,$i,1)) & 0xff;
    $key = HKAES::cipher($pwBytes, HKAES::keyExpansion($pwBytes));
    ...
    $keySchedule = HKAES::keyExpansion($key);
    ...
    for ($b=0; $b<$blockCount; $b++) {
      ...
      $cipherCntr = HKAES::cipher($counterBlock, $keySchedule);  // -- encrypt counter block --
...
#_________________________________________________________________________________________
  ...
  public static function hdecrypt($ciphertext, $password=iAES, $nBits=256) {
    $blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
    if (!($nBits==128 || $nBits==192 || $nBits==256)) return '';  // standard allows 128/192/256 bit keys
